...
class VideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        self.landmarks_files = list(cleaned.values[:, 2]) 
            ...
            for ck in range(self.test_num_segment):
                for cp in range(self.test_num_crop):
                    for idx in range(len(self.label_array)):
                        ...
                        self.test_landmarks_files.append(self.landmarks_files[idx])
    
    def important_feature(self):
        # Eyes
        LEFT_EYE  = [33, 133, 159, 145, 153, 154, 155, 160, 161]
        RIGHT_EYE = [362, 263, 386, 374, 380, 381, 382, 387, 388]

        # Mouth
        MOUTH = [13, 14, 78, 308, 61, 291, 81, 82, 87, 312]

        # Nose
        NOSE = [1, 2, 98, 327, 168]

        # Jaw / Chin
        JAW_CHEEKS = [152, 13, 14, 234, 454, 454, 454]

        # Eyebrows
        LEFT_EYEBROW  = [70, 63, 105, 66, 107, 55]
        RIGHT_EYEBROW = [336, 296, 334, 293, 300, 283]

        # Combine all important landmarks
        IMPORTANT_LANDMARKS = (
            LEFT_EYE +
            RIGHT_EYE +
            MOUTH +
    ...
    def load_landmarks(self, lm_path, frame_indices):
        """
        lm_path       : landmark .npy file path (directly from CSV column 2)
        frame_indices : list/array of int — same indices used for video frames

        Steps:
            1. Load full .npy  shape: (total_frames, 478, 3)  [MediaPipe 478 points]
            2. Select same temporal frames as video  -> (clip_len, 478, 3)
            3. Select only important landmark indices -> (clip_len, N_important, 3)
                where N_important = len(important_feature()) = 47

        Returns:
            torch.FloatTensor  shape: (clip_len, N_important, 3)
                                e.g.  (16, 47, 3)

        If landmark file missing -> returns zeros of same shape (safe fallback)
        """
        # get important landmark indices once
        important_idx = self.important_feature()   # list of ints, len=47
        n_important   = len(important_idx)

        if not self.use_landmarks:
            return torch.zeros(len(frame_indices), n_important, self.landmark_dim)

        if not os.path.exists(lm_path):
            warnings.warn(f"Landmark file not found: {lm_path}. Using zeros.")
            return torch.zeros(len(frame_indices), n_important, self.landmark_dim)

        # Step 1: load full array  (total_frames, 478, 3)

        lm_data         = np.load(lm_path)
        
        if lm_data.ndim == 4 and lm_data.shape[1] == 1:
            lm_data = np.squeeze(lm_data, axis=1) 

        total_lm_frames = lm_data.shape[0]

        # Step 2: select same temporal frames as video  -> (clip_len, 478, 3)
        clipped_indices = np.clip(frame_indices, 0, total_lm_frames - 1).astype(np.int64)
        selected_lm     = lm_data[clipped_indices]        # (clip_len, 478, 3)

        # Step 3: select only important landmark indices -> (clip_len, 47, 3)
        selected_lm = selected_lm[:, important_idx, :]    # (clip_len, N_important, 3)

        return torch.FloatTensor(selected_lm) 
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            lm_path  = self.landmarks_files[index]
            buffer, frame_indices = self.loadvideo_decord(sample, sample_rate_scale=scale_t)
            ...
            # load landmarks for the SAME frames as video
            landmarks = self.load_landmarks(lm_path, frame_indices)  # (clip_len, 47, 3)
            ...
            if args.num_sample > 1:
                ...
                for _ in range(args.num_sample):
                    ...
                    landmark_list.append(landmarks)   # same landmarks for each augmentation
                return frame_list, landmark_list, label_list, index_list, {}
            else:
                buffer = self._aug_frame(buffer, args)

            return buffer, landmarks, self.label_array[index], index, {}

        # ── VALIDATION ─────────────────────────────────────────────────────────
        elif self.mode == 'validation':
            sample  = self.dataset_samples[index]
            lm_path = self.landmarks_files[index]
            buffer, frame_indices = self.loadvideo_decord(sample)
            ...
            return buffer, landmarks, self.label_array[index], sample.split("/")[-1].split(".")[0]

        # ── TEST ───────────────────────────────────────────────────────────────
        elif self.mode == 'test':
            sample   = self.test_dataset[index]
            lm_path  = self.test_landmarks_files[index]
            ...
            # select landmarks for the SAME temporal crop window
            test_frame_indices = frame_indices[temporal_start:temporal_start + self.clip_len]
            landmarks = self.load_landmarks(lm_path, test_frame_indices)  # (clip_len, 47, 3)

            buffer = self.data_transform(buffer)

            return buffer, landmarks, self.test_label_array[index], \
                   sample.split("/")[-1].split(".")[0], chunk_nb, split_nb
    ...
    def loadvideo_decord(self, sample, sample_rate_scale=1):
        ...
        MODIFIED: also returns frame_indices so landmarks can be
        selected for the EXACT same frames.

        Returns:
            buffer       : np.ndarray  (T, H, W, C)
        ...
        if self.mode == 'test':
            ...
            return buffer, np.array(all_index)      # <-- frame_indices return
        ...
        vr.seek(0)
        buffer = vr.get_batch(all_index).asnumpy()
        return buffer, np.array(all_index) 
