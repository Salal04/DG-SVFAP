...
def train_class_batch(model, samples,landmarks, target, criterion):
    outputs = model(samples , landmarks)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples,landmarks ,  targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            loss, output = train_class_batch(
                model, samples,landmarks, targets, criterion)
        else:
...
def final_test(data_loader, model, device, file, save_feature=False):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            # me: for saving feature in the last layer
            if save_feature:
                output, saved_feature = model(videos,landmarks, save_feature=save_feature)
            else:
                output = model(videos , landmarks)
            loss = criterion(output, target)
